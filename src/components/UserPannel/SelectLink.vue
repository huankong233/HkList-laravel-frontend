...
const getFileListClickEvent = async (formEl: FormInstance) => {
  if (!formEl) return
...
const freshFileList = async (formEl: FormInstance) => {
  if (!formEl) return
  if (!(await formEl.validate(() => {}))) return
...
const copyLink = async (formEl: FormInstance) => {
  if (!formEl) return
  if (!(await formEl.validate(() => {}))) return

  copy(
    `${location.host}/?url=${getFileListForm.value.url}&pwd=${getFileListForm.value.password}&dir=${getFileListForm.value.dir}`,
    '复制成功'
  )
}
...
const downloadFile = async (fs_id: number[]) => {
  const response =
...
const saveAria2Config = async (formEl: FormInstance) => {
  if (!formEl) return
  if (!(await formEl.validate(() => {}))) return
